...
public class CategoryClassifier {
	...
	public CategoryClassifier (int qNum, DataSet ambDS) throws JsonParseException, JsonMappingException, IOException, ClassNotFoundException, NoSuchAlgorithmException, KeyManagementException {
		...
		for (int i = 0; i < entityVecs.size(); i++) {
			...
			for (String ent: vec) {
				for (String categ: categs.get(ent)) {
					catData[i] = catData[i] + categ.substring(37);
			//		if (i == 50) System.out.println(ent + " " +catData[i]);
					for (String cat: closures.get(categ))
						catData[i] = catData[i] + cat;
				}
			}
		}
...
//		LingoClusteringAlgorithm.class);
	...
	/*	System.out.println("Input the query term: ");
		...
		for (int i = 3; i < 4; i++) {
			...
			CategoryClassifier catClass = new CategoryClassifier(i, ambDS);
