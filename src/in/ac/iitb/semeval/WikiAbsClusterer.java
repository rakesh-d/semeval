...
public class WikiAbsClusterer {
	...
	public WikiAbsClusterer (int qNum, DataSet ambDS) throws JsonParseException, JsonMappingException, IOException, ClassNotFoundException, NoSuchAlgorithmException, KeyManagementException {
...
//			System.out.println(client.getProperties().keySet());
		...
		absts = new HashMap<String, String>();
		for (String ent: entSet) {
			String query = "SELECT ?o WHERE {<" + ent + "> ?p ?o}";
			q = QueryFactory.create(query);
			qexec = QueryExecutionFactory.create(q, modelAbs);
			ResultSet iteresults = qexec.execSelect();
			while (iteresults.hasNext()) {
				String abs = iteresults.next().get("?o").toString();
				absts.put(ent, abs);
			}
		}
		qexec.close();
...
//		String yagoPrefix = "http://yago-knowledge.org/resource/";
	...
	*/	
		...
		*/
		...
		for (int i = 0; i < entityVecs.size(); i++) {
			...
			catData[i] = "";
			for (String ent: vec) {
				catData[i] = catData[i] + absts.get(ent);
   ...
   //      attributes.put("LingoClusteringAlgorithm.factorizationFactory", NonnegativeMatrixFactorizationEDFactory.class);
         ...
         attributes.put("BisectingKMeansClusteringAlgorithm.clusterCount", 12);
...
//		LingoClusteringAlgorithm.class);
	...
	public static void main (String[] args) throws IOException, ClassNotFoundException, KeyManagementException, NoSuchAlgorithmException {
		...
		String outputFile = "/home/rakesh/Downloads/testdata/abs_clustSTRel.txt";
	...
	/*	System.out.println("Input the query term: ");
		...
		for (int i = 0; i < 100; i++) {
			...
			WikiAbsClusterer abClust = new WikiAbsClusterer(i, ambDS);
			abClust.getClusters();
			abClust.printOutput(pw);
